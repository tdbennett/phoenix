...
and extends the logic from the retrospective data analysis used to develop
the Phoenix criteria.

1. Pick the score time, t. Example: ``We are scoring the patient at 10:15.''
2. Look back only as far as the respiratory look-back window allows. For
   published Phoenix, this is 6 hours.
3. Determine the PFR at time t.
4. Determine the SFR at time t.
5. Select PFR _or_ SFR to score.
   a. If at time t, SFR is defined and PFR is not, use SFR.
   b. If at time t, PFR is defined and SFR is not, use PFR.
   c. If at time t, SFR and PFR are defined, then:
      i. If the age of the PFR value is less than, or no more than one minute
      older, than the age of the SFR value, use the PFR value.
      ii. If the SFR is more than one minute younger than the PFR value, use the
      SFR.
6. Score

**Example:** Minutes into an encounter the observed FiO~2~, PaO~2~, and SpO~2~
values and the carried forward values.  PFR and SFR values.  Notes (below the
table) explain why PFR, SFR, or neigther is used.

 | Minute | FiO~2~ Observerd | FiO~2~ | PaO~2~ Observerd | PAO2 | SPO2 Observerd | SPO2 | PFR    | SFR    | Notes |
 | --:    | --:              | --:    | --:              | --:  | --:            | --:  | --:    | --:    | :--   |
 | 34     |                  |        |                  |      | 100            | 100  |        |        | 1.    |
 | 69     |                  |        |                  |      |                | 100  |        |        | 1.    |
 | 94     | 1                | 1      |                  |      | 99             | 99   |        |        | 2.    |
...
1. No PFR or SFR becuase there is not FiO~2~
2. SpO~2~ is greater than 97 and thus not to be used.  There is no SFR at this time.
3. SpO~2~ is greater than 97 and thus not bo be used.  PaO~2~ observed and is younger than FiO~2~.  Use PFR.
4. SpO~2~ is &leq; 97; can be used to find SFR.  SpO~2~ is younger than FiO~2~.  SFR is younger than PFR.  Use SFR.
5. SFR and PFR are both valid.  PaO~2~ is younger than SpO~2~.  Use PFR.
6. SFR and PFR are both valid.  SpO~2~ is younger than PaO~2~.  Use SFR.
7. SpO~2~ &gt; 97; do not calculate SFR.  PaO~2~ is _older_ than FiO~2~; do not calculate PFR.  There is no valid PFR nor SFR value at this time.
8. SpO~2~ &gt; 97; do not calculate SFR.  PaO~2~ is _younger_ than FiO~2~: calculate PRF and use it.
9. SpO~2~ &leq; 97 and younger than FiO~2~; calculate SFR;  PaO~2~ is _older_ than FiO~2~: do not calculate FiO~2~.  Use SFR.

## Scoring
Scoring is based on PFR _or_ SFR.  If PFR with being used, then the scores are
based on the following limits.  Treat b1, b2, b3 as 0/1 indicators.

|        | PaO~2~ limit | SpO~2~ limit |
| :--:   | :--:         | :--:         |
| b1     | < 400        | < 292        |
| b2     | < 200        | < 220        |
| b3     | < 100        | < 148        |
...
# NOTE (20260827) **DO NOT INCLUDE THIS CHUNK IN THE OUTPUT**  The scoring
# approach has not been implimented in on the main branch of the project yet.
# The implimentation is on the feature-operationalize branch.  Omit this chunk
# for now and add it back when the implimentation has been merged into main.
...
3. Define 4 candidate estimators:
   a. Define Candidate 1.
      i. Find the most recent arterial-line MAP within that window.
      ii. If one exists this is Candidate 1; otherwise Candidate 1 is missing.
   b. Define Candidate 2.
      i. Find the most recent arterial-line SBP and arterial-line DBP within that window.
      ii. Only use arterial SBP and DBP to calculate MAP if they were recorded close enough together (_within 10 minutes of each other_). If they are too far apart in time, do not use them together.
      iii. If arterial SBP and DBP can be used together, calculate: MAP = one third SBP + two thirds DBP. Call this Candidate 2.
   c. Define Candidate 3.
      i. Find the most recent cuff MAP within the window.
      ii. If one exists this is Candidate 3; otherwise Candidate 3 is missing.
   d. Define Candidate 4.
      i. Find the most recent cuff SBP and cuff DBP within the window.
      ii. Only use cuff SBP and DBP to calculate MAP if they were recorded close enough together (_within 10 minutes of each other_). If they are too far apart in time, do not use them together.
...
5. If two candidates are equally new (_within one minute_), use this tie-breaker order:
...
**Example:** The candidate columns have the MAP value and the (value age: how old is the value, in minutes).
